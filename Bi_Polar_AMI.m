...
% Generate Alternate Mark Inversion (AMI) Signal
ami_signal = zeros(1, length(time));
current_level = 1; 
for i = 1:n
    t_index = (i - 1) * samples_per_bit + 1 : i * samples_per_bit; 
    
    if input_signal(i) == 1
        ami_signal(t_index) = current_level;
...
subplot(2,1,2);
plot(time, ami_signal, 'linewidth', 2);
ylabel('AMI Signal');
xlabel('Time (s)'); 
grid on;
axis([0 n -1.5 1.5]);
title('Alternate Mark Inversion (AMI) Signal');
